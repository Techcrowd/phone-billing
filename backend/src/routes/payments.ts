...
router.post('/generate', async (req, res) => {
  ...
  if (invRows.length === 0) return res.status(404).json({ error: 'Faktura nenalezena' });
...
router.get('/', async (req, res) => {
  ...
  if (period) { conditions.push(`i.period = $${idx++}`); params.push(period); }
  if (group_id) { conditions.push(`p.group_id = $${idx++}`); params.push(group_id); }
...
router.get('/export', async (req, res) => {
  ...
  if (period) { sql += ` AND i.period = $${idx++}`; params.push(period); }
  if (group_id) { sql += ` AND p.group_id = $${idx++}`; params.push(group_id); }

  sql += ' ORDER BY i.period DESC, g.name';

  const { rows: payments } = await pool.query(sql, params);
  ...
  const { rows: items } = await pool.query(`
    ...
    FROM invoice_items ii
    JOIN services s ON s.id = ii.service_id
    WHERE ii.invoice_id = ANY($1::int[]) AND s.group_id IS NOT NULL
...
router.put('/:id', async (req, res) => {
  ...
  if (rows.length === 0) return res.status(404).json({ error: 'Platba nenalezena' });

  const paidAt = is_paid ? new Date().toISOString().slice(0, 19).replace('T', ' ') : null;
  await pool.query('UPDATE payments SET is_paid = $1, paid_at = $2 WHERE id = $3', [is_paid, paidAt, id]);
