...
export class PaymentsPage implements OnInit {
  ...
  loadPayments() {
    ...
    this.api.getPayments(period, groupId).subscribe(payments => {
  ...
  togglePaid(payment: Payment, isPaid: boolean) {
    this.api.togglePayment(payment.id, isPaid).subscribe(() => this.loadPayments());
  }

  exportPdf() {
    this.api.downloadPaymentsExport(
